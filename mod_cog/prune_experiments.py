...
if __name__ == '__main__':
    ...
    eg_lrs = [0.1, 0.5, 0.75, 1, 1.25, 1.5, 2.0,  2.5] # 8 x 
    ...
    for m in momentum_arr:
        for pp in prune_props:
            ...
            for lr in eg_lrs:
                for exp in eg_exp_list[:n_seeds]:
                    settings.append((exp, pp, lr, m))
    ...
    cfg.n_epochs = 1
    cfg.use_wandb = True
    cfg.project_dir = Path(os.path.join(os.environ['SCRATCH'], "eg_paper/mod_cog/exp8_pruning_cooldown_0715"))
    cfg.tag = "pruning_cooldown_0715"
    ...
    # Plain GD for the gd arm: sign-freezing off. The sign-constrained (cSGD)
    # version of this experiment is prune_experiments_cSGD.py.
