...
project_dir="$SCRATCH/eg_paper/mod_cog/exp8_final_combined_eg_unsigned" # final /<> becomes the project name on wandb
        ...
        for weight_decay in "${eg_weight_decay_array[@]}"; do
            for momentum in "${momentum_array[@]}"; do
                for seed in "${seeds[@]}"; do
                    run_configs+=("lr=$lr update_alg=eg_unsigned seed=$seed weight_decay=$weight_decay weight_distribution=$distr momentum=$momentum $common_str")
                done
            done
        done
    done
