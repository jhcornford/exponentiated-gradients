...
class EnvWithAdditions(ngym.TrialEnv):
    ...
    def my_set_groundtruth(self, value, period=None, where=None, seq=0):
        ...
        if isinstance(period, str):
          # NB, Mod-Cog quirk kept unchanged: `value` is a choice id (1-16) but the wrap is
          # mod dim_ring (16), so choice 16 is stored as 0 — colliding with fixation in raw
          # label space. We instead fix this at training time (labels + 1, with the decision
          # mask taken from the input fixation channel; see ModCogLossCombined in
          # train_mod_cog.py). Cached data and checkpoints use this label convention;
          # don't "fix" the wrap here without regenerating both.
...
class _DMFamily(EnvWithAdditions):
    ...
    def _add_singlemod(self, trial, mod=1):
        ...
        # stim = _cosinebump(trial['theta1'], self.theta, coh1)
        stim = _gaussianbump(trial['theta1'], self.theta, coh1)
        self.add_ob(stim, period1, where='stimulus' + mod)
        # stim = _cosinebump(trial['theta2'], self.theta, coh2)
...
def dlyantiseqr(**kwargs):
    ...
    env_kwargs['anti'] = True
    env_kwargs['timing'] = {'delay': 500}
    env_kwargs['timing'].update({'decision': 1000})
    env_kwargs['seq'] = 1
    return _reach(**env_kwargs)
...
def construct_mod_cog_dataset(batch_size, seq_len=350):
    envs = get_mod_cog_dataset_envs()
    schedule = RandomSchedule(len(envs)) # todo seed randomschedule for reproducability.
    env = ScheduleEnvs(envs, schedule=schedule, env_input=True)
    dataset = Dataset(env, batch_size=batch_size, seq_len=seq_len, batch_first=True)
    return dataset

def get_mod_cog_dataset_envs():
    ...
    envs = yang_go_anti + mc_go_anti_int + mc_go_seq + mc_anti_seq
    envs += yang_dm + yang_dlydm + mc_dmdly_intl + mc_dmdly_intr + mc_dm_seqr + mc_dm_seql + mc_dmdly_seqr + mc_dmdly_seql
    envs += yang_match + mc_match_intr + mc_match_intl + mc_match_seqr + mc_match_seql

    return envs 

def get_mod_cog_dataset_labels():
    yang_go_anti = ["go", "rtgo", "dlygo", "anti", "rtanti", "dlyanti"]
    ...
    env_strs = yang_go_anti + mc_go_anti_int + mc_go_seq + mc_anti_seq
    env_strs += yang_dm + yang_dlydm + mc_dmdly_intl + mc_dmdly_intr + mc_dm_seqr + mc_dm_seql + mc_dmdly_seqr + mc_dmdly_seql
    env_strs += yang_match + mc_match_intr + mc_match_intl + mc_match_seqr + mc_match_seql
...
if __name__ == "__main__":
    ...
    env = ScheduleEnvs(envs, schedule=schedule, env_input=True)
    dataset = Dataset(env, batch_size=512, seq_len=350, batch_first=True)
    ...
    print(f"inputs size: {inputs.shape}, labels size: {labels.shape}")

    import sys
    np.set_printoptions(threshold=sys.maxsize)
    print(inputs[:,60,33:].sum(axis=0))
    print(inputs[:,:,33:].sum(axis=1).sum(0))
