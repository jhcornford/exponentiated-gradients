"""
Modified from https://github.com/pytorch/pytorch/blob/v2.1.2/torch/nn/utils/prune.py
(torch 2.1, BSD-3-Clause). One change: L1Unstructured has an extra option,
inverse_magnitude (default False). If True, the largest-magnitude weights are
pruned instead of the smallest. Use it via global_unstructured(..., inverse_magnitude=True);
note l1_unstructured() doesn't accept this option. Changes are in L1Unstructured's
docstring, __init__, and the topk call in its compute_mask.
"""
import numbers
...
class BasePruningMethod(ABC):
    ...
    def apply(cls, module, name, *args, importance_scores=None, **kwargs):
        ...
        try:
            ...
            # if not a pruning container, restore the original parameter
            if not isinstance(method, PruningContainer):
...
class PruningContainer(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        r"""This gets called by BasePruningMethod.apply().
...
class L1Unstructured(BasePruningMethod):
    ...
    Args:
        amount (int or float): quantity of parameters to prune.
            ...
            fraction of parameters to prune. If ``int``, it represents the
            absolute number of parameters to prune.
        inverse_magnitude:: Prune weights with the largest absolute values first.
    """

    PRUNING_TYPE = "unstructured"

    def __init__(self, amount, inverse_magnitude=False):
        # Check range of validity of pruning amount
        _validate_pruning_amount_init(amount)
        self.amount = amount
        self.inverse_magnitude = inverse_magnitude 
    ...
    def compute_mask(self, t, default_mask):
        ...
        if nparams_toprune != 0:  # k=0 not supported by torch.kthvalue
            ...
            topk = torch.topk(torch.abs(t).view(-1), k=nparams_toprune, largest=self.inverse_magnitude)
